...
public class E09ForceBook {
    static Map<String, List<String>> users = new LinkedHashMap<>();
    ...
    private static void addUser(String inputString) {
        ...
        if (!doesUserExist) {
            users.putIfAbsent(side, new ArrayList<String>() {{
                add(user);
            }});
    ...
    private static void printUsers() {
        users.entrySet()
                .stream()
                .sorted(Map.Entry.<String, List<String>>comparingByValue((e1, e2) -> Integer.compare(e2.size(), e1.size())).thenComparing(Map.Entry.comparingByKey(Comparator.reverseOrder())))
